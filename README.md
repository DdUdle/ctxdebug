...
| `X64DBG_PIPE_TOKEN` | x64dbg / mco | 256-bit capability token for authenticated x64dbg named-pipe IPC. Gateway generates one automatically for its child processes; standalone/manual x64dbg must share the same token with ctxdebug. |
