...
| `IDA_MCP_TOKEN` | ida | Bearer token for the IDA HTTP server. Authentication is required even on loopback; if unset in IDA, the plugin generates an ephemeral 256-bit token and prints it in the IDA console. |
